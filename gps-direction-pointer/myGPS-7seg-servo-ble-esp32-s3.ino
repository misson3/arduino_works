/* Apr11, 2023 papa-chika: myGPS-7seg-servo-ble-esp32-s3
 * May21, 2022 adding bluetooth for ad hoc location
...
SoftwareSerial mySerial3(A3, A2); // rxPin, txPin ------------------- papachika
...
void setup()
{
  ...
  mySerial3.begin(9600);  // ------------------- papachika
    ...
    if (GPS.fix) {  //---------------------------------------------(1)
      ...
      if (greenState == HIGH) {
        ...
        if (ticker % 5 == 0) {
          ...
          if (abs(servo_pos - prev_pos) >= 20) {  // ------------------ Sep05, 2022
            myservo.attach(servoPin);
      ...
      /*
       * serial reporting to mySerial3 ------------------------------- papachika
       * dest,lat,lon,distance
       */
